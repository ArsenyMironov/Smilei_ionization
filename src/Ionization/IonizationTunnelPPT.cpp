...
IonizationTunnelPPT::IonizationTunnelPPT( Params &params, Species *species ) : Ionization( params, species )
{
    ...
    for( unsigned int Z=0 ; Z<atomic_number_ ; Z++ ) {
        ...
        double Cnl;
        double Blm      = ( 2.*Azimuthal_quantum_number[Z]+1.0 );
        ...
        beta_tunnel[Z] = Cnl*4.*Blm * Potential[Z] * au_to_w0;
...
void IonizationTunnelPPT::operator()( Particles *particles, unsigned int ipart_min, unsigned int ipart_max, vector<double> *Epart, Patch *patch, Projector *Proj, int ipart_ref )
{
...
void IonizationTunnelPPT::ionizationTunnelWithTasks( Particles *particles, unsigned int ipart_min, unsigned int ipart_max, 
